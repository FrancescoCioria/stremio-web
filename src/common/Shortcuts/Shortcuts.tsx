...
const ShortcutsProvider = ({ children, onShortcut }: Props) => {
    ...
    const onKeyDown = useCallback(({ ctrlKey, shiftKey, altKey, metaKey, code, key, repeat }: KeyboardEvent) => {
        // TV fork: Escape (telecomando "B") deve passare anche con focus su
        // input (es. campo ricerca) per fungere da "back" universale. Le
        // altre shortcut restano inibite mentre si digita.
        if (isInputFocused() && code !== 'Escape') return;
